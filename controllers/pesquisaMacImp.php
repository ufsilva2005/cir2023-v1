<?php
    ...
    try {
            $pdo = new Conexao(); 
            $resultado = $pdo->select("SELECT * FROM impressoras WHERE macImpressora = '$parametro'");
            $pdo->desconectar();
    ...
    if(count($resultado))
        {
            foreach ($resultado as $res) 
                {
                    echo "<script language = javascript> alert('JÁ EXISTE UMA IMPRESSORA NO CADASTRO COM ESTE MAC !!!' );</script>";
